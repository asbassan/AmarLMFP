...
Open `Chapter1_Predict_Next_Token.ipynb` and follow along. Every code block below corresponds to a cell in the notebook.
...
The round-trip works. `'h'` is at index 72 because ASCII 104, sorted position 72 in our 95-character vocabulary. Every character has exactly one integer, and every integer maps back to exactly one character. No information is lost.
...
**Cross-entropy loss** is the standard objective for next-token prediction. It measures how well the model's probability distribution over the vocabulary matches the actual next character. A random model over 95 characters would assign equal probability to all of them, giving a cross-entropy of log(95) ≈ 4.55 bits. Our starting loss of 4.98 is close to that — the untrained model is nearly random.
...
A>
A> We actually ran a controlled experiment on this corpus (and two others) to quantify exactly how much each character benefits from context beyond one preceding character. The findings are discussed in the companion paper *Which Characters Need Context?* — structural characters like punctuation benefit more from longer context than letters do, at least in prose. The bigram is the model whose limitations that paper measures.
...
`block_size = 8` in this chapter. In LLaMA 3 the context window is 128,000 tokens. The concept is the same: how many preceding tokens can the model attend to when predicting the next one? This number has direct consequences for inference. A longer context window means larger KV-caches, more memory bandwidth consumed per token, and higher latency per request. The bigram's effective context of one character is the limiting case — it tells you exactly what a model loses when it can see almost nothing. Every subsequent chapter in this series extends that window deliberately, and each extension has a cost you will be able to reason about.

### Autoregressive Generation

The `generate_greedy()` function is the foundation of autoregressive inference. Every production language model generates text the same way: predict the next token, append it, predict again. The loop structure is identical. What changes in production systems is:
...
The "the the the the" loop is not just a curiosity. It is the reason temperature and top-p parameters exist in every inference API you will ever use. Understanding the failure mode of greedy decoding from first principles means you know what those parameters are actually fixing.

### Perplexity in Production

The perplexity metric you computed — `exp(loss)` — is used to benchmark production models. When a paper reports that a model achieves perplexity of 6.1 on WikiText-103, it is computing the same quantity: averaged cross-entropy, exponentiated. Your bigram's perplexity of 12.3 on Shakespeare gives you a concrete intuition for what the number means — the model is as uncertain as if it were choosing uniformly among 12 characters. A well-trained modern model on the same task would be uncertain among far fewer options.

### Inference Engineering

Inference engineering is the discipline of making generation fast, cheap, and reliable at scale. The concepts it cares about — context length management, batching strategy, decoding algorithm, memory footprint — all trace directly back to the mechanisms in this chapter. Understanding why `block_size` matters, why greedy decoding fails, and why batch construction works the way it does gives you the foundation to reason about inference optimisation as models and serving systems grow more complex throughout this series.
...
The data pipeline built in this chapter does not change as we move to more powerful models. The same `get_batch` function, the same train/val split, the same device pattern will carry through every subsequent chapter.

What changes is what happens between `get_batch` and `loss.backward()`.

**Chapter 01b** replaces the bigram's embedding lookup with a simple feedforward model that can see multiple preceding characters. The loss drops further, and the generated text starts to look less random.

**Chapter 01c** addresses the greedy collapse with proper sampling strategies. Temperature scaling, top-k, and nucleus sampling each give the model room to be creative rather than deterministic.

By Chapter 03 we will have replaced the feedforward with a self-attention layer — the mechanism that makes transformers transformers.
